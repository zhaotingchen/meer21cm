...
def get_3d_power_multipole(
    ...
    # Use the repo's built-in multipole estimator
    mock.k1dweights = k_xy_sel.astype(float)

    beam_att = mock.beam_attenuation()
    """except Exception:
        # If beam attenuation isn't available for some mock configurations,
        # fall back to no attenuation (keeps previous behavior).
        beam_att = 1.0"""

    pdata3d_beam = pdata3d.copy() * (beam_att**2)
    pcross3d_beam = pcross3d.copy() * beam_att
    ...
    # Compute multipole moments for HI (ell=0,2,4) using beam-attenuated mock power
    pdata_ell0, keff, nmodes = get_multipole(pdata3d_beam, 0)
    pdata_ell2, _, _ = get_multipole(pdata3d_beam, 2)
    pdata_ell4, _, _ = get_multipole(pdata3d_beam, 4)

    # Analytical model multipoles (models may already include beam)
    ...
    # Compute cross multipole moments (ell=0,2,4) using beam-attenuated mock power
    pcross_ell0, _, _ = get_multipole(pcross3d_beam, 0)
    pcross_ell2, _, _ = get_multipole(pcross3d_beam, 2)
    pcross_ell4, _, _ = get_multipole(pcross3d_beam, 4)
    ...
    # Compute 2D cylindrical power spectra (use beam-attenuated mocks for pdata and pcross)
    pdata_cy = bin_power_cy(
        pdata3d_beam[None], mock.k_perp, mock.k_para, kperpbins, kparabins
    )[0]
    ...
    pcross_cy = bin_power_cy(
        pcross3d_beam[None], mock.k_perp, mock.k_para, kperpbins, kparabins
    )[0]
