...
class ForegroundSimulation():
    ...
    def sync_map(self):
        ...
        sync_map,self.sync_nside,self.sync_unit,self.sync_freq = read_healpix_fits(file)
        if check_unit_equiv(self.sync_unit,units.Jy):
            if self.verbose:
                print("convert "+file+" to temperature")
            sync_map = (
                sync_map * self.sync_unit/1/units.Jy
            ).to('').value
            sync_map = convert_hpmap_in_jy_to_temp(
                sync_map,
                self.sync_freq
            )
        return sync_map
    ...
    def sync_spindx(self):
        ...
        if sync_indx_file is None:
            ...
            if (
                isinstance(sync_indx_file[0],str) and 
                isinstance(sync_indx_file[1],str)
            ):
            ...
            raise TypeError(
                'cannot recognise sync_indx_file'
            )
        if use_map:
            ...
            sp_indx = (
                np.log(unit_conv*map1_val/map2_val)
                /np.log(map1_freq/map2_freq)
            )
        else:
            sp_indx = np.ones(hp.nside2npix(self.sync_nside))*uni_indx
        return sp_indx
    def fg_cube(self,freq,source_type='sync',wproj=None,xdim=None,ydim=None,sigma_beam_ch=None):
        if source_type == 'sync':
        ...
        if sigma_beam_ch is not None:
            for ch_id in range(out_map.shape[-1]):
                out_map[:,ch_id] = hp.smoothing(
                    out_map[:,ch_id],
                    sigma=sigma_beam_ch[ch_id]*np.pi/180
                )
