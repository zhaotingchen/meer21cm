...
def gen_random_gal_pos(
    wproj, W_HI, num_g, ra_range=(-np.inf, np.inf), dec_range=(-400, 400), rng=None
):
    """
...
def run_poisson_mock(
    ...
    ra_g_mock, dec_g_mock, inside_range = gen_random_gal_pos(
        wproj, W_HI[:, :, 0], num_g, ra_range=ra_range, dec_range=dec_range, rng=rng
    )
    ...
    if not fast_ang_pos:
        ...
        vertices = [
            hp.ang2vec(
                ra_temp.min() - hp_map_extend, dec.min() - hp_map_extend, lonlat=True
            ),
            hp.ang2vec(
                ra_temp.min() - hp_map_extend, dec.max() + hp_map_extend, lonlat=True
            ),
            hp.ang2vec(
                ra_temp.max() + hp_map_extend, dec.max() + hp_map_extend, lonlat=True
            ),
            hp.ang2vec(
                ra_temp.max() - hp_map_extend, dec.min() - hp_map_extend, lonlat=True
            ),
        ]
        ...
        himap_g = np.concatenate(
            (himap_g[:, 0][:, None], himap_g, himap_g[:, 0][:, None]), axis=-1
        )
    else:
        himap_g = np.zeros(ra.shape + nu.shape)
        # add a filler channel for both start and end of frequency
        himap_g = np.concatenate(
            (himap_g[:, :, 0][:, :, None], himap_g, himap_g[:, :, 0][:, :, None]),
            axis=-1,
        )
