...
def stack(
    ...
    for gal_i in range(num_g):
        ...
        if ignore_double_counting:
            ...
            gal_sel_indx = (((sel_stack==gal_i)+(sel_stack==-1)).astype('bool')).astype('float')
    ...
    if verbose:
        stack_2D_map = (
            np.nan_to_num(stack_3D_map).sum(axis=(-1))
        )
        ...
        plt.show()
        stack_1D_map = (
            np.nan_to_num(stack_3D_map).sum(axis=(0,1))
        )
        plt.stairs(stack_1D_map,x_edges)
        plt.xlabel(r'$\Delta _z$ ['+f"{x_unit:latex}"+']')
        plt.show()
    if return_indx_and_weight:
        return stack_3D_map,stack_3D_weight,x_edges,ang_edges,map_gal_indx,map_gal_weight
    else:
